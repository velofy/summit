...
function searchModal() {
  return `<div class="search-root" s-data="search" data-base="../"
    ...
    <div class="search-panel" s-show="isOpen" @keydown.escape.window="close()" @keydown.down.prevent="move(1)" @keydown.up.prevent="move(-1)" @keydown.enter.prevent="go()">
      ...
      <div class="search-results" s-ref="list">
        <template s-if="query && results.length === 0">
          <div class="search-empty">No results for "<span s-text="query"></span>".</div>
        </template>
        <template s-if="!query && recent.length">
          <div class="search-section">Recent</div>
          <template s-for="(r, i) in recent" :key="'r'+i">
            <a class="search-hit" :href="r.url" @mouseenter="active = i" @click="remember(r)">
        ...
        <template s-for="(r, i) in results" :key="r.url + i">
          <a class="search-hit" :class="{ active: i === active }" :href="r.url" @mouseenter="active = i" @click="remember(r)">
            <span class="hit-icon">#</span>
            <span class="hit-main">
...
<header class="topbar">
  ...
  <button class="top-icon" title="Toggle theme" aria-label="Toggle theme" s-data @click="document.documentElement.dataset.theme=(document.documentElement.dataset.theme==='dark'?'light':'dark');localStorage.setItem('summit-theme',document.documentElement.dataset.theme)"><span s-text="document.documentElement.dataset.theme==='dark'?'☀':'☾'">☾</span></button>
